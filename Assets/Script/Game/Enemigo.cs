...
public class Enemigo : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if(targetSombi != null)
        {
            ...
            if (targetSombi.GetComponent<ZombieLife>().vidaZombie <= 0)
            {
                frambuesa += Time.deltaTime;
                if(frambuesa >= 2)
                {
    ...
    void FixedUpdate()
    {
        ...
        {
            //Es
            if(detectarPlayer || detectarSombi || !patrulla.patrullaActivada)
            {
                reactivarPatrol += Time.deltaTime;
                patrulla.patrullaActivada = false;
                patrulla.puntosUbi = 0;
                detectarSombi = false;
                detectarPlayer = false;
                if(reactivarPatrol >= 4)
                {
            ...
            //ConPatrulla true;
            //Condicion que solo lo active con la patrulla = un bool en patrulla 
            
            if (patrulla.patrullaActivada)
            {
                ...
                //countFuego < limiteCountFuego hay que fijarse en dos variables:
                //1.La posición a la que se dirige el enemigo con las ubicaciones de Patrulla
                //2.El numero de ubicaciones que quedan posterior al fuego.
                //Con estas dos variables puedes rellenar "limiteCountFuego" que representa el número de ubicaciones que se encuentra posterior al fuego.
